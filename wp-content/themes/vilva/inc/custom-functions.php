...
function vilva_scripts() {
    ...
    if ( get_theme_mod( 'ed_localgoogle_fonts', false ) && ! is_customize_preview() && ! is_admin() && get_theme_mod( 'ed_preload_local_fonts', false ) ) {
        vilva_preload_local_fonts( vilva_fonts_url() );
    }
    ...
    if( is_post_type_archive( 'product' ) ){
        ...
        if( is_category() ){
            ...
            if( $ed_prefix ){
                ...
                $title = sprintf( __( '%1$s Year: %2$s %3$s', 'vilva'), '<span class="sub-title">','</span>', '<h1 class="date-title">' . get_the_date( _x( 'Y', 'yearly archives date format', 'vilva' ) ) . '</h1>') ;
            }
        }elseif ( is_month() ) {
            if( $ed_prefix ){
...
function vilva_admin_notice(){
    ...
    $dismissnonce   = wp_create_nonce( 'vilva_admin_notice' );
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><a href="<?php echo esc_url( admin_url( 'themes.php?page=vilva-dashboard' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Go to the dashboard.', 'vilva' ); ?></a></p>
                    <p class="dismiss-link"><strong><a href="?vilva_admin_notice=1&_wpnonce=<?php echo esc_attr( $dismissnonce ); ?>"><?php esc_html_e( 'Dismiss', 'vilva' ); ?></a></strong></p>
                </div>
            </div>
        </div>
    <?php }
...
function vilva_update_admin_notice(){
    ...
    if ( isset( $_GET['vilva_admin_notice'] ) && $_GET['vilva_admin_notice'] = '1' && wp_verify_nonce( $_GET['_wpnonce'], 'vilva_admin_notice' )) {
